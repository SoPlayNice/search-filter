		...
		public function check_posts()
		{
			...
			if((isset($_POST[SF_FPRE.'category']))&&($this->has_form_posted))
			{
				...
				if(count($catarr)>0)
				{
					...
					if(get_option('permalink_structure')&&($taxcount==0))
					{
						...
						$category_path = $category_base."/".$categories."/";  // Added a slash so it is pretty
					...
					else
					{
						if(!$this->hasqmark)
						{
							$this->urlparams .= ""; // Instead of adding a question mark after the slash, we do nothing
							$this->hasqmark = true;
						}
						else
						{
							$this->urlparams .= "/"; // Instead of adding and & for each query, we add a slash to make the urls pretty
						}
						
						$this->urlparams .= "category_name/".$categories; // Added a slash so it is pretty
					}
			...
			if((isset($_POST[SF_FPRE.'post_tag']))&&($this->has_form_posted))
			{
				...
				if(count($tagarr)>0)
				{
					...
					if(get_option('permalink_structure')&&($taxcount==0))
					{
						$tag_path = "tag/".$tags."/";  // Added a slash so it is pretty
					...
					else
					{
						...
						$this->urlparams .= "tag/".$tags;  // Added a slash so it is pretty
					}
					...
					{//if the key is not in the reserved array (ie, on a custom taxonomy - not tags, categories, search term, post type & post date)
						...
						if(count($taxarr)>0)
						{
							...
							{	
								...
								$tax_path = $key."/".$taxs."/";  // Added a slash so it is pretty
							...
							else
							{
								...
								$this->urlparams .=  $key_taxonomy->rewrite['slug']."/".$taxs;  // Added a slash so it is pretty. Also told the url to be the rewritten slug so it returns dashes in the name instead of underscores.
							}
			...
			if((isset($_POST[SF_FPRE.'post_types']))&&($this->has_form_posted))
			{
				...
				if(count($post_types_arr)>0)
				{
					...
					$this->urlparams .= "post_types/".$post_types;  // Added a slash so it is pretty
				...
				if(count($post_date_arr)>0)
				{
					...
					if(isset($post_date))
					{
						if($post_date!="")
						{
							...
							$this->urlparams .= "post_date/".$post_date;  // Added a slash so it is pretty
						}
					}
				}
			}
